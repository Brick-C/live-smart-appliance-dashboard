...
// ENHANCED SUMMARY SECTION - Timeframe Aware
// Integrate this with your main.js for complete historical integration

class EnhancedSummaryManager {
  constructor() {
    this.currentTimeframe = "today";
    this.deviceCapabilities = {
      bfb4193c3c41952bd25rju: "power-monitoring", // Deep Freezer
      bf3ef2ae093d5173c3yma5: "switch-only", // Computer (LDNIO SCW1050)
    };
  }

  async updateSummaryForTimeframe(timeframe, deviceId) {
    console.log(
      `📊 Updating summary for timeframe: ${timeframe}, device: ${deviceId}`
    );

    this.currentTimeframe = timeframe;
    const deviceType = this.deviceCapabilities[deviceId] || "unknown";

    try {
      let statsData = null;

      if (deviceType === "switch-only") {
        statsData = this.getSwitchOnlyStats(timeframe);
      } else {
        statsData = await this.getHistoricalStats(deviceId, timeframe);
      }

      this.displaySummary(statsData, timeframe, deviceType);
      this.updateSummaryLabels(timeframe);
    } catch (error) {
      console.error(`❌ Error updating summary for ${timeframe}:`, error);
      this.displayError(timeframe);
    }
  }

  async getHistoricalStats(deviceId, timeframe) {
    const now = new Date();
    ...
    console.log(
      `📊 Fetching stats for ${timeRange}: ${startTime.toDateString()} - ${endTime.toDateString()}`
    );
    ...
    return {
      ...
      deviceType: this.deviceCapabilities[deviceId],
    };
  }

  getSwitchOnlyStats(timeframe) {
    // For switch-only devices, return appropriate stats
    return {
      peakWatts: 0,
      avgWatts: 0,
      totalKWh: 0,
      totalCost: 0,
      timeframe: this.getPeriodLabel(timeframe),
      deviceType: "switch-only",
      message: "Switch-only device - No power monitoring available",
    };
  }

  displaySummary(stats, timeframe, deviceType) {
    console.log(`📊 Displaying summary for ${timeframe}:`, stats);

    const peakElement = document.getElementById("peak-usage");
    const avgElement = document.getElementById("avg-usage");
    const totalElement = document.getElementById("total-usage");
    const costElement = document.getElementById("total-cost");

    // Update summary values
    if (peakElement)
      peakElement.textContent = `${(stats.peakWatts || 0).toFixed(1)}W`;
    if (avgElement)
      avgElement.textContent = `${(stats.avgWatts || 0).toFixed(1)}W`;
    if (totalElement)
      totalElement.textContent = `${(stats.totalKWh || 0).toFixed(3)} kWh`;
    if (costElement)
      costElement.textContent = `৳${(stats.totalCost || 0).toFixed(2)}`;

    // Add visual indicator for device type
    this.addDeviceTypeIndicator(deviceType, stats.message);
  }

  updateSummaryLabels(timeframe) {
    const labels = {
      today: {
        peak: "Peak Usage",
        avg: "Average Usage",
    ...
    const currentLabels = labels[timeframe] || labels["today"];

    // Update label text (you can add specific elements or keep as is)
    console.log(`📋 Summary labels updated for ${timeframe}`);
  }

  addDeviceTypeIndicator(deviceType, message) {
    // Remove existing indicator
    const existing = document.getElementById("device-type-summary");
    if (existing) existing.remove();

    // Add new indicator
    const indicator = document.createElement("div");
    indicator.id = "device-type-summary";
    ...
    if (deviceType === "switch-only") {
      ...
      indicator.textContent = "🔌 Switch-only device";
    } else {
      indicator.style.backgroundColor = "#ECFDF5";
      indicator.style.color = "#065F46";
      indicator.style.border = "1px solid #6EE7B7";
      indicator.textContent = "⚡ Power monitoring enabled";
    }

    // Append to summary section
    const summarySection = document.querySelector(".space-y-3");
    if (summarySection) {
      summarySection.appendChild(indicator);
    }
  }

  displayError(timeframe) {
    const peakElement = document.getElementById("peak-usage");
    const avgElement = document.getElementById("avg-usage");
    const totalElement = document.getElementById("total-usage");
    const costElement = document.getElementById("total-cost");
  ...
  getPeriodLabel(timeframe) {
    ...
    return labels[timeframe] || "Today";
  }
}

// INTEGRATION WITH MAIN TIMEFRAME CHANGES
class IntegratedDashboard {
  constructor() {
    this.summaryManager = new EnhancedSummaryManager();
    this.mainCharts = this.initializeMainCharts();
  }
  ...
  async handleTimeframeChange(timeframe) {
    console.log(`🔄 Handling timeframe change to: ${timeframe}`);

    try {
      // Update summary section
      await this.summaryManager.updateSummaryForTimeframe(
        timeframe,
        currentDeviceId
      );

      // Update main charts based on timeframe
      await this.updateMainChartsForTimeframe(timeframe);

      // Update historical section
  ...
  async updateMainChartsForTimeframe(timeframe) {
    console.log(`📊 Updating main charts for timeframe: ${timeframe}`);

    if (timeframe === "today") {
      // Show live data - charts already do this by default
      this.restoreLiveChartMode();
    } else {
      // Show historical data
      await this.switchToHistoricalChartMode(timeframe);
    }
  }

  async switchToHistoricalChartMode(timeframe) {
    console.log(`📊 Switching to historical chart mode: ${timeframe}`);

    try {
      // Get historical data for main charts
      const historicalData = await this.getMainChartsHistoricalData(timeframe);

      // Update power draw chart
      this.updatePowerDrawChart(historicalData, timeframe);

      // Update energy trend chart
      this.updateEnergyTrendChart(historicalData, timeframe);

      // Add timeframe indicator to charts
      this.addChartTimeframeIndicator(timeframe);
    } catch (error) {
      console.error(`❌ Error switching to historical mode:`, error);
    }
  }

  async getMainChartsHistoricalData(timeframe) {
    const now = new Date();
    ...
    switch (timeframe) {
      ...
      default:
        return null; // Today - keep live mode
    }
  ...
  updatePowerDrawChart(data, timeframe) {
    if (!data || !data.readings) return;

    console.log(`📊 Updating power draw chart for ${timeframe}`);

    // Process data for the chart
    const chartData = this.processDataForTimeframe(data.readings, timeframe);

    // Update the chart (adjust based on your chart library)
    if (typeof powerChart !== "undefined") {
      powerChart.data.labels = chartData.labels;
      powerChart.data.datasets[0].data = chartData.values;
      powerChart.update();
    }
  }

  updateEnergyTrendChart(data, timeframe) {
    if (!data || !data.readings) return;

    console.log(`📊 Updating energy trend chart for ${timeframe}`);

    // Process energy data
    const chartData = this.processEnergyDataForTimeframe(
      data.readings,
      timeframe
    );
    ...
    if (typeof energyChart !== "undefined") {
      ...
      energyChart.data.datasets[0].data = chartData.values;
      energyChart.update();
    }
  }

  processDataForTimeframe(readings, timeframe) {
    if (timeframe === "yesterday") {
      // Hourly data for yesterday
      const hourlyData = Array(24).fill(0);
      readings.forEach((reading) => {
        const hour = new Date(reading.timestamp).getHours();
        hourlyData[hour] = reading.watts || 0;
      });
      ...
      const dailyData = this.aggregateDailyData(readings);

      return {
        labels: dailyData.map((d) => d.date),
        values: dailyData.map((d) => d.avgWatts),
      };
    }
  }

  processEnergyDataForTimeframe(readings, timeframe) {
    // Similar to power but for energy consumption
    return this.processDataForTimeframe(readings, timeframe);
  }

  aggregateDailyData(readings) {
    const dailyGroups = {};

    readings.forEach((reading) => {
      const date = new Date(reading.timestamp).toDateString();
      if (!dailyGroups[date]) {
        dailyGroups[date] = { readings: [], date };
      }
    ...
    return Object.values(dailyGroups).map((group) => ({
      date: group.date,
      avgWatts:
        group.readings.reduce((sum, r) => sum + (r.watts || 0), 0) /
        group.readings.length,
      totalKWh: group.readings.reduce((sum, r) => sum + (r.kWh || 0), 0),
    }));
  }

  addChartTimeframeIndicator(timeframe) {
    // Add visual indicator to main charts
    const periodLabel = this.summaryManager.getPeriodLabel(timeframe);

    // You can add indicators to chart titles, axes, or overlays
    console.log(`📊 Added timeframe indicator: ${periodLabel} to main charts`);
  }

  restoreLiveChartMode() {
    console.log(`📊 Restoring live chart mode`);

    // Remove historical data indicators
    this.removeTimeframeIndicators();

    // Charts will continue showing live data by default
  }

  removeTimeframeIndicators() {
    // Remove any historical mode indicators
    console.log(`📊 Removed historical mode indicators`);
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (timeframeSelect) {
    timeframeSelect.addEventListener("change", async function () {
      ...
      await integratedDashboard.handleTimeframeChange(this.value);
